    ...
    <div className="rounded-xl border border-sand bg-white">
      ...
      <div className="flex items-center justify-between border-b border-sand px-4 py-3">
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-medium text-forest">Editor de entregable</span>
          </div>
          {/* Auto-save status */}
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            disabled={saving}
            className="flex items-center gap-2 rounded-full bg-gradient-primary px-3 py-1.5 text-sm font-medium text-white transition-colors hover:opacity-90 disabled:opacity-50"
          >
            <Save size={14} />
      ...
      {preview ? (
        ...
        <textarea
          ...
          className="min-h-[400px] w-full resize-y bg-transparent p-4 font-mono text-sm text-forest placeholder:text-stone focus:outline-none"
...
function MarkdownPreview({ content }: { content: string }) {
  if (!content.trim()) {
    return <p className="text-sm text-stone italic">Sin contenido.</p>;
  }

  const lines = content.split("\n");

  return (
    <div className="space-y-2 text-sm text-forest leading-relaxed">
      {lines.map((line, i) => {
        ...
        if (trimmed.startsWith("### ")) {
          return (
            <h4 key={i} className="font-display text-base font-700 text-forest mt-4">
        ...
        if (trimmed.startsWith("## ")) {
          return (
            <h3 key={i} className="font-display text-lg font-700 text-forest mt-5">
        ...
        if (trimmed.startsWith("# ")) {
          return (
            <h2 key={i} className="font-display text-xl font-800 text-forest mt-6">
        ...
        if (trimmed.startsWith("- ") || trimmed.startsWith("* ")) {
          return (
            <p key={i} className="pl-4">
              <span className="text-coral mr-2">&bull;</span>
              {applyInline(trimmed.slice(2))}
            </p>
          );
        }
...
function applyInline(text: string): React.ReactNode {
  ...
  return parts.map((part, i) => {
    if (part.startsWith("**") && part.endsWith("**")) {
      return (
        <strong key={i} className="font-medium text-forest">
          {part.slice(2, -2)}
        </strong>
      );
    }
