...
const tool = TOOL_DEFINITIONS["politica-medioambiental"];

export default function PoliticaMedioambientalPage() {
  return (
...
function PoliticaMedioambiental() {
  ...
  async function handleSubmit(data: Record<string, unknown>) {
    ...
    setError(null);
    try {
      const res = await fetch("/api/tools/politica-medioambiental/checkout", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        ...
        <div className="mx-auto max-w-2xl px-6">
          ...
          {generating && (
            <div className="rounded-2xl border border-sand bg-white p-8 text-center shadow-sm">
              ...
              <p className="mt-4 text-sm text-mid">Generando tu política medioambiental...</p>
            </div>
          )}
          ...
          {!result && !generating && status !== "success" && (
            <div className="rounded-2xl border border-sand bg-white p-6 shadow-sm">
              ...
              <ToolForm steps={tool.steps} onSubmit={handleSubmit} loading={loading} submitLabel="Pagar 99€ y generar" />
            </div>
          )}

          {result && (
            <ToolResult title="Tu Política Medioambiental" upsell={tool.upsell}>
