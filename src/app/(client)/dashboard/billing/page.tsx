    ...
    <div>
      ...
      <div className="mt-8 grid gap-4 sm:grid-cols-3">
        <div className="rounded-xl border border-steel/30 bg-smoke p-5">
          <span className="text-sm text-pale">Total pagado</span>
          <p className="mt-2 font-display text-2xl font-700 text-off-white">
            {loading ? "…" : formatEUR(totalPaid / 100)}
          </p>
        </div>
        <div className="rounded-xl border border-steel/30 bg-smoke p-5">
          <span className="text-sm text-pale">Facturas</span>
          <p className="mt-2 font-display text-2xl font-700 text-off-white">
            {loading ? "…" : invoices.length}
          </p>
        </div>
        <div className="rounded-xl border border-steel/30 bg-smoke p-5">
          <span className="text-sm text-pale">Pendientes</span>
          <p className="mt-2 font-display text-2xl font-700 text-off-white">
        ...
        <div className="mt-8 space-y-3">
          {invoices.map((invoice) => {
            ...
            return (
              <div
                key={invoice.id}
                className="flex items-center justify-between rounded-xl border border-steel/30 bg-smoke p-5"
              >
                <div className="flex items-center gap-4">
                  <FileText size={20} className="text-pale" />
                  <div>
